...
void TCPSender::fill_window() {
    ...
    if (_window_size) {
	    ...
	    size_t payload_size = min({_stream.buffer_size(),
			               static_cast<size_t>(_free_space),
...
void TCPSender::ack_received(const WrappingInt32 ackno, const uint16_t window_size) {
	...
	if (!_segments_outstanding.empty()) {
		_free_space = static_cast<uint16_t>(abs_ackno + static_cast<uint64_t>(window_size) -
		                                    unwrap(_segments_outstanding.front().header().seqno, _isn, _next_seqno) - _bytes_in_flight);                                                                                        }
	if (!_bytes_in_flight) _timer_running = false;
